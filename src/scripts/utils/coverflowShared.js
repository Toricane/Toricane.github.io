/** Shared coverflow helpers (build + browser). */
...
export function getCoverflowImageSources(originalPath) {
  if (!originalPath) {
...
/** Deterministic LCP pick: stable sort by path, first image. */
export function pickLcpFaceImage(faceImages) {
...
/** HTML for a single static hero coverflow card (build-time LCP). */
export function renderStaticLcpCardHtml(image, colors = {}) {
